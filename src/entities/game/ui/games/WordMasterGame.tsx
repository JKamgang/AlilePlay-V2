...
const WordMasterGame: React.FC<WordMasterGameProps> = ({ t }) => {
  ...
  const handlePlayWord = () => {
    ...
    const placedCoords = Array.from(placements.keys()).map(key => key.split(',').map(Number));
    ...
    const word = Array.from(placements.values()).map(p => p.tile).join('');
  ...
  const currentPlacedWord = useMemo(() => Array.from(placements.values()).map(p => p.tile).join(''), [placements]);
