...
const TetrisGame: React.FC<{ game: Game; options: any }> = ({ game }) => {
  ...
  const spawnPiece = useCallback((currentBoard?: (string | null)[][]) => {
    ...
    for (let r = 0; r < shape.length; r++) {
      for (let c = 0; c < shape[r].length; c++) {
        if (shape[r][c] && (targetBoard[newPiece.pos.r + r]?.[newPiece.pos.c + c])) {
          setGameOver(true);
          return null;
        }
      }
  ...
  const resetGame = useCallback(() => {
    const cleanBoard = Array(ROWS).fill(null).map(() => Array(COLS).fill(null));
    setBoard(cleanBoard);
    setScore(0);
    setGameOver(false);
    setIsPaused(false);
    const first = spawnPiece(cleanBoard);
  ...
  const mergePiece = useCallback(() => {
    ...
    setCurrentPiece(spawnPiece(filteredBoard));
  ...
  }, [moveDown, gameOver, isPaused, score, currentPiece]);
  ...
  return (
    <div className="flex flex-col md:flex-row items-center justify-center gap-8 max-w-4xl mx-auto p-4 animate-fade-in">
          ...
          {board.map((row, r) =>
            row.map((cell, c) => {
              ...
              return (
                <div
                  key={`${r}-${c}`}
                  className={`w-6 h-6 sm:w-8 sm:h-8 ${type ? COLORS[type] : 'bg-gray-900'} border border-black/10 transition-colors duration-100`}
                />
              );
            })
          )}
      ...
      <div className="flex flex-col items-center space-y-6 w-full md:w-48">
        <div className="bg-light-surface dark:bg-dark-surface p-6 rounded-2xl w-full text-center shadow-lg border border-gray-100 dark:border-gray-800">
        ...
        <button
          ...
          className="w-full py-4 bg-gray-100 dark:bg-gray-800 rounded-2xl font-bold hover:bg-gray-200 dark:hover:bg-gray-700 transition-all active:scale-95 shadow-md"
        >
