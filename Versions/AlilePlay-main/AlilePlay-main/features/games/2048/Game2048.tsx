...
const Game2048: React.FC<{ game: Game; options: any }> = ({ game }) => {
  ...
  const handleTouchEnd = (e: React.TouchEvent) => {
    ...
    if (Math.abs(dx) > Math.abs(dy)) {
      if (Math.abs(dx) > minSwipeDistance) move(dx > 0 ? 'RIGHT' : 'LEFT');
    } else {
      if (Math.abs(dy) > minSwipeDistance) move(dy > 0 ? 'DOWN' : 'UP');
    }
  ...
  return (
    <div className="flex flex-col items-center max-w-md mx-auto p-6 bg-light-surface dark:bg-dark-surface rounded-3xl shadow-2xl">
      ...
      <div onTouchStart={handleTouchStart} onTouchEnd={handleTouchEnd} className="relative bg-gray-300 dark:bg-gray-700 p-3 rounded-2xl grid grid-cols-4 gap-3 w-80 h-80 shadow-inner overflow-hidden touch-none">
        {grid.flat().map((val, i) => (
          <div key={i} className={`w-16 h-16 flex items-center justify-center text-2xl font-black rounded-xl transition-all duration-150 transform ${getTileColor(val)}`}>
            {val}
          </div>
        ))}
        {gameOver && (
          <div className="absolute inset-0 bg-black/70 rounded-2xl flex flex-col items-center justify-center animate-fade-in backdrop-blur-md">
            ...
            <button onClick={initializeGame} className="bg-brand-primary text-white px-10 py-3 rounded-xl font-black shadow-lg hover:bg-brand-secondary transition-all transform active:scale-95 uppercase tracking-widest">{t('play_again')}</button>
          </div>
        )}
      </div>
    </div>
