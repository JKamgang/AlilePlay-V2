...
const LudoGame: React.FC<{ game: Game; options: any }> = ({ game }) => {
  ...
  const moveToken = (tokenId: number) => {
    ...
    // "Requires a 6 to exit base" logic
    if (token.position === 'base') {
        if (diceRoll === 6) nextPos = 0; // Move to starting tile
        else return;
  ...
  return (
    <div className="flex flex-col items-center p-6 bg-light-surface dark:bg-dark-surface rounded-3xl shadow-2xl max-w-2xl mx-auto border-4 border-brand-primary/10">
        ...
        {/* Ludo Board Track Visuals */}
        <div className="relative w-full max-w-sm aspect-square bg-white dark:bg-gray-900 border-8 border-gray-100 dark:border-gray-900 grid grid-cols-15 grid-rows-15 shadow-2xl rounded-xl overflow-hidden">
            {/* Base Zones */}
            <div className="absolute top-0 left-0 w-[40%] h-[40%] bg-red-500/10 border-r-4 border-b-4 border-red-500 flex flex-wrap p-4 gap-2 justify-center items-center">
                {tokens.filter(t => t.color === 'red' && t.position === 'base').map(t => (
                    <div key={t.id} onClick={() => moveToken(t.id)} className={`w-10 h-10 rounded-full bg-red-500 border-4 border-white shadow-xl cursor-pointer hover:scale-110 active:scale-90 transition-all ${diceRoll === 6 && currentPlayer === 'red' ? 'animate-bounce ring-4 ring-red-400' : ''}`} />
                ))}
            </div>
            <div className="absolute top-0 right-0 w-[40%] h-[40%] bg-green-500/10 border-l-4 border-b-4 border-green-500 flex flex-wrap p-4 gap-2 justify-center items-center">
                {tokens.filter(t => t.color === 'green' && t.position === 'base').map(t => (
                    <div key={t.id} onClick={() => moveToken(t.id)} className={`w-10 h-10 rounded-full bg-green-500 border-4 border-white shadow-xl cursor-pointer hover:scale-110 active:scale-90 transition-all ${diceRoll === 6 && currentPlayer === 'green' ? 'animate-bounce ring-4 ring-green-400' : ''}`} />
                ))}
            </div>
            <div className="absolute bottom-0 left-0 w-[40%] h-[40%] bg-blue-500/10 border-r-4 border-t-4 border-blue-500 flex flex-wrap p-4 gap-2 justify-center items-center">
                {tokens.filter(t => t.color === 'blue' && t.position === 'base').map(t => (
                    <div key={t.id} onClick={() => moveToken(t.id)} className={`w-10 h-10 rounded-full bg-blue-500 border-4 border-white shadow-xl cursor-pointer hover:scale-110 active:scale-90 transition-all ${diceRoll === 6 && currentPlayer === 'blue' ? 'animate-bounce ring-4 ring-blue-400' : ''}`} />
                ))}
            </div>
            <div className="absolute bottom-0 right-0 w-[40%] h-[40%] bg-yellow-500/10 border-l-4 border-t-4 border-yellow-500 flex flex-wrap p-4 gap-2 justify-center items-center">
                {tokens.filter(t => t.color === 'yellow' && t.position === 'base').map(t => (
                    <div key={t.id} onClick={() => moveToken(t.id)} className={`w-10 h-10 rounded-full bg-yellow-500 border-4 border-white shadow-xl cursor-pointer hover:scale-110 active:scale-90 transition-all ${diceRoll === 6 && currentPlayer === 'yellow' ? 'animate-bounce ring-4 ring-yellow-400' : ''}`} />
                ))}
            </div>
            ...
            <div className="absolute inset-[40%] bg-white dark:bg-gray-800 border-4 border-gray-400 flex items-center justify-center overflow-hidden">
                ...
                <span className="absolute font-black text-[10px] uppercase text-gray-800 dark:text-gray-100 bg-white/80 dark:bg-black/60 px-2 rounded-full">Home</span>
            </div>

            {/* Track Borders (Visual Clarity) */}
            <div className="absolute top-[40%] left-0 w-[40%] h-[20%] border-y-2 border-brand-primary/10 bg-gray-50/50 dark:bg-white/5" />
            <div className="absolute top-0 left-[40%] w-[20%] h-[40%] border-x-2 border-brand-primary/10 bg-gray-50/50 dark:bg-white/5" />
            <div className="absolute bottom-[40%] right-0 w-[40%] h-[20%] border-y-2 border-brand-primary/10 bg-gray-50/50 dark:bg-white/5" />
            <div className="absolute bottom-0 right-[40%] w-[20%] h-[40%] border-x-2 border-brand-primary/10 bg-gray-50/50 dark:bg-white/5" />
        ...
        <div className="mt-8 p-4 bg-brand-primary/5 rounded-2xl border border-brand-primary/10 max-w-sm">
